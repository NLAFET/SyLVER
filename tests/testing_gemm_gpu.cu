...
namespace tests {   
   ...
   cudaError_t cutlass_gemm_test<float>(
      ...
      //    float, float, float,
      //    cutlass::gemm::LinearScaling<float>, // functor to do the math in the epilogue
   ...
   cudaError_t cutlass_wmma_gemm_test<half>(
         cudaStream_t stream,
         int m, int n, int k, half alpha,
         half *a, int lda, half *b, int ldb, half beta, half *c, int ldc) {
      ...
      typedef cutlass::gemm::WmmaGemmTraits<
         ...
         // cutlass::Shape<128, 128, 128>,            // threadblock tile size
         half, half, half
         // cutlass::gemm::LinearScaling<half>, // functor to do the math in the epilogue
         // half // accumulator type
         // cutlass::Shape<128, 32, 32>,           // warp tile size
         // cutlass::Shape<16, 16, 16>,            // WMMA instruction tile size
         // 8,                                    // scalars every time a thread loads from A
         // 8                                     // scalars every time a thread loads from B
         >
         GemmTraits;
