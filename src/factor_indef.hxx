...
void factor_subtree_indef(
      NumericFrontType& root,
      int n,
      int nnodes,
      std::vector<NumericFrontType>& fronts,
      void** child_contrib,
      FactorAlloc& factor_alloc,
      PoolAlloc& pool_alloc,
      typename NumericFrontType::ValueType const* aval,
   ...
   for (int ni = least_desc; ni <= rootidx; ++ni) {
      ...
      NumericFrontType& front = fronts[ni];         
...
template <typename PoolAlloc, typename NumericFrontType>
void factor_front_indef_notask(
   ...
   using FactorAlloc = typename NumericFrontType::FactorAlloc;
   ...
   if (options.pivot_method==sylver::PivotMethod::app_block) {
      ...
      using Backup = sylver::CopyBackup<ValueType, PoolAlloc>; 
      // Disable debug mode
      bool const debug = false;
      // Inner block size
      int const ib = INNER_BLOCK_SIZE;
      // Define factorization type 
      using FactorType = FactorIndefAPP<
         ValueType,
         ib,
         Backup,
         debug,
...
void factor_front_indef(
   ...
   int m = node.nrow();
   int n = node.ncol();
   size_t ldl = align_lda<ValueType>(m);
   ValueType *lcol = node.lcol;
   ValueType *d = &node.lcol[n*ldl];
   int *perm = node.perm;
...
#if defined(SPLDLT_USE_STARPU)
   sylver::spldlt::starpu::codelet_init_indef<T, iblksz, Backup, FactorAlloc, PoolAlloc>();
...
class FactorIndefAPP {
   ...
   typedef typename std::allocator_traits<PoolAlloc>::template rebind_alloc<T> TAlloc;
   // Block type
   using BlockType = spldlt::ldlt_app_internal::Block<T, iblksz, IntAlloc>;
   // Numeric front type
   using NumericFrontType = sylver::spldlt::NumericFront<T, FactorAlloc, PoolAlloc>;
   /// \}
protected:
   ...
   */ 
   ...
   static void applyN_block_app_task(
         BlockType& dblk, BlockType& rblk,
         sylver::ColumnData<T,IntAlloc>& cdata, Backup& backup,
         sylver::options_t& options) {
   ...
   */
   ...
   static void updateT_block_app_task(            
         BlockType& isrc, BlockType& jsrc, BlockType& ublk,            
   ...
   */
   ...
   int factorize_indef_app_notask (
         ...
         PoolAlloc const& alloc, int const from_blk=0) {

      typedef spldlt::ldlt_app_internal::Block<T, iblksz, IntAlloc> BlockType;
      ...
      for(int blk=from_blk; blk<nblk; blk++) {
         ...
         // {
         // BlockType dblk(blk, blk, m, n, cdata, a, lda, block_size);
            
         // Factorize block on diagonal

         // BlockType dblk(blk, blk, m, n, cdata, &a[blk*block_size*lda+blk*block_size], lda, block_size);
         ...
         for(int jblk=0; jblk<blk; jblk++) {
            ...
            // BlockType cblk(blk, jblk, m, n, cdata, &a[jblk*block_size*lda+blk*block_size], lda, block_size);               
            ...
            BlockType& cblk = blocks[jblk*mblk+blk];
         ...
         for(int iblk=blk+1; iblk<mblk; iblk++) {
            ...
            // BlockType rblk(iblk, blk, m, n, cdata, &a[blk*block_size*lda+iblk*block_size], lda, block_size);
            ...
            BlockType& rblk = blocks[blk*mblk+iblk];
         ...
         for(int jblk=0; jblk<blk; jblk++) {
            ...
            // BlockType jsrc(blk, jblk, m, n, cdata, &a[jblk*block_size*lda+blk*block_size], lda, block_size);
            BlockType& jsrc = blocks[jblk*mblk+blk];

            for(int iblk=jblk; iblk<mblk; iblk++) {

               int isrc_row = (blk<=iblk) ? iblk : blk;
               int isrc_col = (blk<=iblk) ? blk : iblk;
               // BlockType isrc(isrc_row, isrc_col, m, n, cdata, &a[isrc_col*block_size*lda+isrc_row*block_size], lda,
               // block_size);
               // BlockType ublk(iblk, jblk, m, n, cdata, &a[jblk*block_size*lda+iblk*block_size], lda, block_size);

               BlockType& ublk = blocks[jblk*mblk+iblk];
               BlockType& isrc = blocks[isrc_col*mblk+isrc_row];
         ...
         for(int jblk=blk; jblk<nblk; jblk++) {
            ...
            // Source block
            // BlockType jsrc(jblk, blk, m, n, cdata, &a[blk*block_size*lda+jblk*block_size], lda, block_size);

            BlockType& jsrc = blocks[blk*mblk+jblk];

            for(int iblk=jblk; iblk<mblk; iblk++) {

               // Source block
               // BlockType isrc(iblk, blk, m, n, cdata, &a[blk*block_size*lda+iblk*block_size], lda, block_size);
               // Destination block
               // BlockType ublk(iblk, jblk, m, n, cdata, &a[jblk*block_size*lda+iblk*block_size], lda, block_size);

               BlockType& ublk = blocks[jblk*mblk+iblk];
               BlockType& isrc = blocks[blk*mblk+iblk];
...
public:
   ...
   void factor_front_indef_app_notask(
      ...
      int* perm = front.perm;
      auto& blocks = front.blocks;
      auto& contrib_blocks = front.contrib_blocks;
      auto& cdata = *front.cdata;
      auto& backup = *front.backup;
         ...
         if (contrib_dimn > 0) {
            ...
            for (int jblk = rsa; jblk < mblk; ++jblk) {
               for (int iblk = jblk;  iblk < mblk; ++iblk) {
                  ...
                  sylver::Tile<T, PoolAlloc>& upd = front.contrib_block(iblk, jblk);
   ...
   void factor_front_indef_app(
         ...
         T const beta, T* upd, int const ldupd,
         std::vector<spral::ssids::cpu::Workspace>& workspaces,
         PoolAlloc const& alloc, int& next_elim, int const from_blk=0
         ) {
      ...
      auto& contrib_blocks = node.contrib_blocks;
